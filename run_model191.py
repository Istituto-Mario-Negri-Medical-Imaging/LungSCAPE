...
OUTPUT_SUFFIX = "_highAttenuation191.nii.gz"
...
def deploy_outputs(output_dir: Path, data_dir: Path, nnunet_input_dir: Path) -> None:
    ...
    then saves each output as DATA/PatientID/PatientID_highAttenuation191.nii.gz.
    """
...
def cleanup(paths: dict) -> None:
    ...
    log("The pretrained model was removed (nnUNet_results). Run setup_model191.py again before the next segmentation.")


def main():
    parser = argparse.ArgumentParser(
        description="Run nnU-Net high attenuation segmentation (Dataset191) using OUTPUT folder inside the project base directory."
    )
    parser.add_argument(
        "base_dir",
        help="Project base directory created by setup_model191.py",
    )
    parser.add_argument(
        "--project-dir",
        default=None,
        ...
        help=(
            ...
            "DATA/PatientID/PatientID_highAttenuation191.nii.gz using case_mapping.json."
        ),
    )
    parser.add_argument(
        "--no-cleanup",
    ...
    try:
        ...
        if auto_mode:
            ...
            print(f"High attenuation outputs automatically deployed to {data_dir}")
        else:
            print(f"Segmentation output saved in: {paths['OUTPUT']}")
            print("Each output must be renamed to {PatientID}_highAttenuation191.nii.gz and placed")
        ...
        if args.no_cleanup:
            ...
            print("Run setup_model191.py again before the next segmentation.")
