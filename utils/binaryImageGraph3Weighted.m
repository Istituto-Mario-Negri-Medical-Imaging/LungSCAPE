function [g,nodenums] = binaryImageGraph3Weighted(map,conn)
%binaryImageGraph3Weighted  Weighted graph of foreground pixel connectivity in 3-D binary image
%
%   Extension of binaryImageGraph3 from the Image Graphs toolbox
%   (Steve Eddins, MathWorks File Exchange #53614). This version adds
%   intensity-based edge weights: each edge weight is the Euclidean
%   distance between voxels plus the neighbor's intensity value from the
%   input map. Requires the Image Graphs toolbox to be on the MATLAB path
%   (uses requireR2015b and checkConnectivity).
%
%   g = binaryImageGraph3Weighted(MAP)
%   g = binaryImageGraph3Weighted(MAP,conn)
%
%   [g,nodenums] = binaryImageGraph3Weighted(___)
%
%   g = binaryImageGraph3Weighted(MAP) computes the 26-connected pixel
%   neighbor graph for the foreground pixels of a 3-D image, with
%   weighted edges.
%
%   g = binaryImageGraph3Weighted(MAP,conn) computes the pixel neighbor
%   graph using the specified connectivity.
%
%   [g,nodenums] = binaryImageGraph3Weighted(___) also returns an array
%   the size of MAP whose elements are the graph numbers corresponding
...
%
%   See also binaryImageGraph3, binaryImageGraphWeighted,
%   plotImageGraph, imageGraph3.
...
for k = 1:length(conn_offsets_x)
   edge_table = [...
      edge_table
      binaryImageGraphNeighborEdgesWeighted(bw,map,nodenums,...
...
function edge_table = binaryImageGraphNeighborEdgesWeighted(bw,map,node_number_image,foreground_pixel_indices,foreground_x,foreground_y,foreground_z,dx,dy,dz)
t = table;
